...
const Cart = () => {
    ...
    //const [products, setCart, userid] = useState([])

    fetchCartContent();

    // const getProducts = () => {
    //     axios.get(`https://cna-cart-api.herokuapp.com/cart/${userid}`, { headers: { Authorization: `Bearer ${access_token}` }} )
    //     .then((response) => {
    //         if(response === ""){
    //             console.log("wow ")
    //         }
    //         else{
    //             console.log("got cart ", response)

    //         }
    //     })
    // } 

    // const what = getProducts();
    ...
    //const productEls = what.map(e => <Product key={e.id} id={e.id}name={e.name} desc={e.description} price={e.price} rating={e.rating} />)
    //{productEls}
    return(
